...
The pipeline requires **Python 3.10 or higher** and a few third-party libraries. We do not need to install the Typst compiler binary globally because the script uses embedded Python bindings for Typst compilation.

---

## 2. Installation

We install the required packages using our Python package manager. Since the project contains a `uv.lock` file, it is highly recommended to use **`uv`**:
...
uv sync
```
Or, if we want to install them in our active environment:
```bash
...
If we receive a compile error like `failed to load file (access denied)`, we ensure the `root="."` argument is enabled in the compiler call in `generate_journal.py`. Typst restricts reading files outside the directory of the entrypoint file, and passing the project root allows it to load the `templates/` folder from inside `renders/`.

### Font Loading Warnings
Typst automatically queries the host system's installed fonts. The default body font is **EB Garamond**, with **DejaVu Sans Mono** and **Segoe UI Symbol** used for grids and icons. If these are missing on the machine, Typst will fall back to default serif/monospace system fonts. We can configure alternative fallback fonts in `config.json`.
