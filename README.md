...
```mermaid
graph TD
    subgraph RM [Traditional Paradigm]
        A[User: Independent Observer] --> B(Mirror: Page) --> C[Reflection: Passive Record]
    end
    subgraph DI [The Trajectory Trace]
        D[User] <-->|Intra-Active Cut: Ink on Paper| E[The Material Page]
        E --> F[Interference Pattern: Diffracted State]
    end
    style E fill:#0f172a,stroke:#38bdf8,stroke-width:1px
    style F fill:#0f172a,stroke:#10b981,stroke-width:1px
```
...
```mermaid
graph TD
    subgraph HI [8 Operational Operators]
        O1[A1: Attractor] --- O2[S1: Stabilizer]
        O3[F1: Flight] --- O4[R1: Resistance]
        O5[V1: Vitality] --- O6[D1: Diffraction]
        O7[C1: Chronos] --- O8[A2: Aion]
    end
    
    subgraph PM [Projection Layer]
        W[16D Manifold Projection Matrix]
    end

    subgraph SS [16D Computational Space]
        S1[Homeostatic s01]
        S2[Amplifying s02]
        S3[Cyclic s03]
        S4[Bifurcated s04]
        S5[Decentralized s05]
        S6[Rhizomatic s06]
        S7[Boundary Perm. s07]
        S8[Recursion Depth s08]
        S9[Variety Filtering s09]
        S10[Negentropic Comp. s10]
        S11[Temporal Latency s11]
        S12[Attractor Depth s12]
        S13[Symbiotic s13]
        S14[Nomadic s14]
        S15[Co-Orientation s15]
        S16[Substrate Mater. s16]
    end

    O1 & O2 & O3 & O4 & O5 & O6 & O7 & O8 --> W
    W --> SS
    
    style HI fill:#0f172a,stroke:#38bdf8,stroke-width:1px
    style PM fill:#0f172a,stroke:#10b981,stroke-width:1px
    style SS fill:#0f172a,stroke:#a78bfa,stroke-width:1px
```
...
```mermaid
graph LR
    config[config.json] --> gen[generate_journal.py]
    content[journal_content.json] --> gen
    templates[templates/*.typ] --> gen
    gen -->|Decoupled Generation| compiled[renders/compiled_journal.typ]
    compiled -->|Embedded Typst Compiler| pdf[renders/compiled_journal.pdf]
    
    style gen fill:#0f172a,stroke:#38bdf8,stroke-width:1px
    style compiled fill:#0f172a,stroke:#10b981,stroke-width:1px
    style pdf fill:#0f172a,stroke:#a78bfa,stroke-width:2px
```
